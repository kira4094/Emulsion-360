...
use gelatin::glium::{
    ...
    texture::{MipmapsOption, SrgbTexture2d},
...
pub struct SphereViewer {
    ...
    program: Program,
    /// LRU cache: Vec of (path, texture), MRU at the end, owns all textures
    cache: Vec<(PathBuf, SrgbTexture2d)>,
...
impl SphereViewer {
    pub fn new(display: &gelatin::Display) -> Self {
        ...
        SphereViewer {
            ...
            cache: Vec::new(), max_cache_size: 3, current_path: None,
    ...
    /// Load a panorama from file with LRU caching and mipmap support.
    pub fn load_panorama(&mut self, display: &gelatin::Display, path: &PathBuf) {
        // Bump MRU order if already in cache
        ...
        match gelatin::image::open(path) {
            Ok(img) => {
                ...
                let tex = SrgbTexture2d::with_mipmaps(
                    display, raw, MipmapsOption::AutoGeneratedMipmaps
                ).unwrap();
                // Evict LRU if cache is full
                while self.cache.len() >= self.max_cache_size {
                    self.cache.remove(0);
                }
                self.cache.push((path.clone(), tex));
                self.current_path = Some(path.clone());
            }
            Err(e) => {
    ...
    pub fn draw(
        ...
        let tex = match self.current_path.as_ref()
            .and_then(|p| self.cache.iter().rev().find(|(cp, _)| cp == p))
            .map(|(_, t)| t)
        {
            Some(t) => t,
            None => return,
